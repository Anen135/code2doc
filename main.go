...
func main() {
	...
	logger.Info(fmt.Sprintf("API Key configured, model: %s", model))
	...
	if err != nil {
		...
		logger.Error(fmt.Sprintf("Scan failed: %v", err))
		return
	}

	logger.Info(fmt.Sprintf("Found %d files to process", len(files)))
	...
	for _, file := range files {
		...
		if err != nil {
			...
			logger.Error(fmt.Sprintf("Failed to analyze code: %v", err))
			continue
		}
		...
		if err != nil {
			...
			logger.Error(fmt.Sprintf("Failed to generate doc for %s: %v", file.Path, err))
			continue
		}
